...
def WndProc(hWnd, uMsg, wParam, lParam):
    ...
    if uMsg == WM_COPYDATA:  # WM_COPYDATA
        ...
        if message == "Hello!!":
            logger.debug(f"DLL: (WndProc) Hello")
            Hello = True
            return 1
        if message == "slsurl":
            logger.debug(f"DLL: (WndProc) slsurl")
            Sls = True
            return 1
        if message == "gamestr":
            logger.debug(f"DLL: (WndProc) gamestr")
            GameStr = True
            return 1
        if message.startswith("endPopup"):
            logger.debug(f"DLL: (WndProc) endPopup")
        ...
        if message.startswith("getWebLinkUrl"):
            ...
            return 1
        if MessageListener is not None:
            logger.debug(f"DLL: (WndProc) {message}")
...
def RegisterMessageListener(f):
    logger.debug(f"DLL: Registered Message Listener")
    global MessageListener
    MessageListener = f


def SendMessageToClient(responseTo, message):
    logger.debug(f"DLL: {responseTo} | {message}")
